...
| `04_piano_favorites.sql`           | 11         | RLS self-only. RPCs : toggle_piano_favorite (idempotent, advisory_lock, ban guard), get_my_favorites. **Régression bug Sprint 7 : last_update_at**                               |
...
| Bug                                                                                              | Détection                                                                     | Fix                                                                |
| ------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| `get_my_favorites.last_update_at` référence `pu.updated_at` (n'existe pas)                       | Sprint 7 sécu — détecté à l'exécution Supabase SQL Editor                     | `fix(v7): get_my_favorites colonne pu.updated_at -> pu.created_at` |
| `queue_favorite_update_notification` référence `new.quality` (n'existe pas, c'est `new_quality`) | Sprint 9 — détecté par test 04 piano_favorites (premier INSERT piano_updates) | Inclus dans Sprint 9                                               |
