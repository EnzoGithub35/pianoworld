...
`initSentry()` est appelé eagerly au top-level ([src/main.tsx:13](../src/main.tsx#L13)). Le CSS Leaflet est importé globalement ([src/main.tsx:11](../src/main.tsx#L11)) — pas dans `MapPage`, donc le carto CSS est shippé partout (gotcha bundle).
...
| `/dashboard`    | `Dashboard`       | `RequireAuth`                                                                    | `AppShell`                       | 5 tabs                                                                     |
| `/search`       | `SearchPage`      | `RequireAuth`                                                                    | `AppShell`                       | v7 PR-B unifiera users + pianos                                            |
| `/settings`     | `SettingsPage`    | `RequireAuth`                                                                    | `AppShell`                       |                                                                            |
| `/user/:pseudo` | `UserPage`        | `RequireAuth`                                                                    | `AppShell`                       |                                                                            |
...
**4 items aujourd'hui** ([NavBar.tsx:5-10](../src/components/Layout/NavBar.tsx#L5-L10)) :

1. `/dashboard` — `LayoutDashboard` — "Accueil"
...
Fixed at `inset-x-0 bottom-0 z-50`, `bg-background/95` + `backdrop-blur-md` avec fallback `supports-[backdrop-filter]`, safe-area-bottom via inline `paddingBottom: env(safe-area-inset-bottom)` ([NavBar.tsx:14-17](../src/components/Layout/NavBar.tsx#L14-L17)). État actif : barre primaire top `h-0.5 w-8` + icône `scale-110` ([NavBar.tsx:35-46](../src/components/Layout/NavBar.tsx#L35-L46)).

Pas de système de badge dans NavBar — les badges vivent dans les triggers de tab dans `Dashboard` ([Dashboard.tsx:50-56](../src/pages/Dashboard.tsx#L50-L56) pour `newReplyCount`, [Dashboard.tsx:41](../src/pages/Dashboard.tsx#L41) pour `pendingFriendsCount`).

**v7 PR-B** ajoutera une 5e icône `Users` → `/friends` (page standalone wrappant FriendsTab) + Badge `usePendingReceivedCount()` sur l'item. Dashboard tab "Amis" sera retiré (déplacé), tab "Favoris" ajouté à la place.
...
| Page           | File                         | Lazy | Note                                                                                                                                                                                                                                  |
| -------------- | ---------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ------------------------------------------------------------------------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MapPage`      | `src/pages/MapPage.tsx`      | ✓    | `<PianoMap />` full-bleed + FAB `+` (z-500) + `<Tutorial />`                                                                                                                                                                          |
| `Dashboard`    | `src/pages/Dashboard.tsx`    | ✓    | **5 tabs** (`activity                                                                                                                                                                                                                 | community | events                                                                   | requests | friends`). `?tab=`searchParam deep-link bidirectionnel ([Dashboard.tsx:34-48, 69-74](../src/pages/Dashboard.tsx#L34-L48)).`activity` clear le param. Badges via localStorage (`REQUESTS_LAST_SEEN_KEY`) + `usePendingReceivedCount()`. ⚠️ Comment JSDoc en tête dit encore "3 onglets" (stale). |
| `SearchPage`   | `src/pages/SearchPage.tsx`   | ✓    | Pseudo search via `useUserSearch`. Gate `USER_SEARCH_MIN_CHARS`. v7 PR-B la transforme en SearchTabs (Users + Pianos).                                                                                                                |
| `SettingsPage` | `src/pages/SettingsPage.tsx` | ✓    | Sections : compte, notifications, theme, admin link, RGPD, logout. Imports `useFriends` + `usePendingReceivedCount`.                                                                                                                  |
| `UserPage`     | `src/pages/UserPage.tsx`     | ✓    | `:pseudo` param. Hooks `useProfileByPseudo` + `useUserPianos`. `<AddFriendButton />` v6.                                                                                                                                              |
| `PianoPage`    | `src/pages/PianoPage.tsx`    | ✓    | **Public read**. Owner-only edit/delete CTAs. `<PianoPresenceCounter variant="page" />` v6.                                                                                                                                           |
...
| `LegalPage`    | `src/pages/LegalPage.tsx`    | ✓    | **Publique**. 3 sub-tabs (`mentions                                                                                                                                                                                                   | privacy   | cgu`) via URL hash. `CookieBanner.tsx:64`deep-link vers`/legal#privacy`. |
| `AdminPage`    | `src/pages/AdminPage.tsx`    | ✓    | `RequireAdmin`. 7 onglets en mémoire (KPIs, users, reports, events, requests, audit, **Roles** si superadmin). Tabs **non URL-synced** — gotcha refresh.                                                                              |
...
**v7 PR-B** ajoutera : `usePianoSearch`, `useEmailSearch`, `useFavorites` (+ `useIsFavorited`, `useToggleFavorite`).
...
### Structure du schema.sql (3057 lignes après v7)
...
| 15      | v7 Recherche + favoris (extensions, profiles cols, indexes GIN trgm, piano_favorites, notify_favorite_update, 6 RPCs, export étendu)                                                                             | 2471-3004       |
