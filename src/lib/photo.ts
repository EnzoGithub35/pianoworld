...
export function validatePhotoFile(file: File): void {
  ...
  if (sizeMB > MAX_INPUT_SIZE_MB) {
    throw new Error(`Image trop lourde (${sizeMB.toFixed(1)} Mo, max ${MAX_INPUT_SIZE_MB} Mo)`)
  }
}
...
export async function deletePianoPhoto(publicUrl: string): Promise<void> {
  ...
  if (error) {
    logger.warn('photo.delete', 'remove failed (non-fatal)', { path, error: error.message })
  } else {
